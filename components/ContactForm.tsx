...
export default function ContactForm({ locale = 'el' }: ContactFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      newErrors.name =
        locale === 'el' ? 'Το όνομα είναι υποχρεωτικό' : 'Name is required';
    ...
    if (!formData.phone.trim()) {
      newErrors.phone =
        locale === 'el' ? 'Το τηλέφωνο είναι υποχρεωτικό' : 'Phone is required';
    } else if (
      !/^(\+30|0030|30)?[0-9]{10}$/.test(formData.phone.replace(/\s/g, ''))
    ) {
      newErrors.phone =
        locale === 'el'
          ? 'Παρακαλώ εισάγετε έγκυρο ελληνικό τηλέφωνο'
          : 'Please enter a valid phone number';
    }

    if (!formData.message.trim()) {
      newErrors.message =
        locale === 'el' ? 'Το μήνυμα είναι υποχρεωτικό' : 'Message is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(
          locale === 'el' ? 'Κάτι πήγε στραβά' : 'Something went wrong'
        );
      }
    } catch (error) {
      ...
      setSubmitMessage(
        locale === 'el'
          ...
          : 'Sorry, there was a problem sending. Please try again or call us.'
      );
    } finally {
    ...
    <div className='bg-white rounded-xl shadow-lg p-8 max-w-2xl mx-auto'>
      <div className='text-center mb-8'>
        ...
        <p className='text-gray-600'>
          {locale === 'el'
            ...
            : 'Send us your message and we will contact you immediately'}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
        ...
        <div>
          ...
          <input
            ...
            placeholder={locale === 'el' ? 'Το όνομά σας' : 'Your name'}
        ...
        <div>
          ...
          <input
            ...
            placeholder={
              locale === 'el'
                ? '210 123 4567 ή +30 210 123 4567'
                : '+30 210 123 4567'
            }
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={
              locale === 'el'
                ? 'Περιγράψτε την εκδρομή που επιθυμείτε...'
        ...
        >
          {isSubmitting ? (
            <span className='flex items-center justify-center space-x-2'>
              ...
              <span>{locale === 'el' ? 'Αποστολή...' : 'Sending...'}</span>
            </span>
          ) : locale === 'el' ? (
            'Στείλε Μήνυμα'
          ) : (
      ...
      <div className='mt-8 pt-6 border-t border-gray-200 text-center'>
        <p className='text-gray-600 mb-2'>
          {locale === 'el'
            ? 'Ή επικοινωνήστε μαζί μας άμεσα:'
